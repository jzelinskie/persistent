...
func TestSimpleMap(t *testing.T) {
	...
	m1 := &validatedMap{
		impl:     NewMap[int, int](cmp.Less[int]),
...
func dfsRef(node *mapNode[int, int], countByEntry map[mapEntry]int32, nodesByEntry map[mapEntry]map[*mapNode[int, int]]struct{}) {
	if node == nil {
		return
	}

	entry := mapEntry{key: node.key, value: node.value.value}
...
func (vm *validatedMap) validate(t *testing.T) {
	...
	validateNode(t, vm.impl.root, cmp.Less[int])
...
func validateNode(t *testing.T, node *mapNode[int, int], less func(a, b int) bool) {
	if node == nil {
		return
	}
...
func (vm *validatedMap) set(t *testing.T, key, value int) {
	...
	vm.impl.Set(key, value, func(deletedKey, deletedValue int) {
